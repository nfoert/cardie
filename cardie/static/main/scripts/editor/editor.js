...
async function start_editor() {
    if (demo_param === false) {
        const uuid_param = new URL(window.location.href).searchParams.get(
            "uuid",
        );

        document.querySelector("#editor_main_preview_demo").style.display =
            "none";
        ...
        if (uuid_param == null) {
            ...
            response.text().then((text) => {
                if (text === "Done") {
                    const refresh = `${window.location.protocol}//${window.location.host}${window.location.pathname}?uuid=${new_uuid}`;
        ...
        document.querySelector("#editor_main_preview_share").style.display =
            "none";
    }
}
...
async function editor_demo_auth(sign_in) {
    ...
    response.text().then((text) => {
        if (text === "Missing headers") {
            ...
            log(
                "INFO",
                `The new temporary card has been created with uuid ${text}!`,
            );
...
document
    .querySelector("#editor_main_settings_layout_buttons_left")
    .addEventListener("click", (event) => {
...
document
    .querySelector("#editor_main_settings_layout_buttons_center")
    .addEventListener("click", (event) => {
...
document
    .querySelector("#editor_main_settings_layout_buttons_right")
    .addEventListener("click", (event) => {
